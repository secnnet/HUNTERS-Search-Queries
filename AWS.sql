--This is a series of 7 AQL (Amazon Query Language) queries that retrieve data from different tables within a database called ICON_EU_DB.RAW.
...
--So, each of these queries retrieves the first 10 rows of data from a different table in the ICON_EU_DB.RAW database. These tables contain information related to various Amazon Web Services (AWS) services such as CloudTrail, Config, Elastic Load Balancing (ELB), GuardDuty, Inspector Findings, VPC Flow Logs, and Web Application Firewall (WAF).

SELECT *
FROM ICON_EU_DB.RAW.AWS_CLOUDTRAIL
Limit 10

SELECT *
FROM ICON_EU_DB.RAW.AWS_CONFIG
Limit 10

SELECT *
FROM ICON_EU_DB.RAW.AWS_ELB_CLASSIC
Limit 10

SELECT *
FROM ICON_EU_DB.RAW.AWS_GUARD_DUTY
Limit 10

SELECT *
FROM ICON_EU_DB.RAW.AWS_INSPECTOR_FINDINGS
Limit 10

SELECT *
FROM ICON_EU_DB.RAW.AWS_VPC_FLOW_LOGS
Limit 10

SELECT *
FROM ICON_EU_DB.RAW.AWS_WAF
Limit 10